...
AdversarialShield can also be hosted on Hugging Face Spaces for a free portfolio demo.

1. Push the trained checkpoint to the Hugging Face Hub.

```bash
python - <<'EOF'
from transformers import AutoModelForSequenceClassification, AutoTokenizer

model = AutoModelForSequenceClassification.from_pretrained("./model/checkpoints/best_model")
tokenizer = AutoTokenizer.from_pretrained("./model/checkpoints/best_model")

model.push_to_hub("YOUR_USERNAME/adversarialshield")
tokenizer.push_to_hub("YOUR_USERNAME/adversarialshield")
print("✅ Model pushed to Hugging Face Hub")
EOF
```

2. Create a Gradio or Streamlit Space and load the model from the Hub at startup.

3. The free CPU tier is enough for demos, and you can upgrade to GPU later if needed.

If you keep the model in a Space, the UI can load it directly from the Hub when the app starts.

## 🔌 API Usage (OpenAI-compatible middleware)

```python
import httpx

# Check a prompt BEFORE sending to your LLM
response = httpx.post("http://localhost:8000/v1/classify",
    json={"text": user_prompt})
result = response.json()

if result["is_threat"]:
    return f"Blocked: {result['explanation']}"
else:
    return openai_client.chat(user_prompt)  # safe to proceed
```
